    ...
    public class Startup
    {
        ...
        public Startup(IHostingEnvironment env)
        {
            ...
            if ((!string.IsNullOrEmpty(Configuration["IsAuditEnable"])) && Convert.ToBoolean(Configuration["IsAuditEnable"]) == true)
            {
                ...
                if (!string.IsNullOrEmpty(encPwd) && IsBase64String(encPwd))  //check is it encrypted or not
                {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            if (!string.IsNullOrEmpty(encPassword) && IsBase64String(encPassword))
            {
            ...
            if (!string.IsNullOrEmpty(encPwd_Admin) && IsBase64String(encPwd_Admin))
            {
            ...
            // Removed legacy RazorViewEngineOptions CompilationCallback logic
        }

        // This method gets called by the runtime. Use this method to configure the HTTP request pipeline.
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, ILoggerFactory loggerFactory)
        {
            // Logging is configured in appsettings.json or via ILoggingBuilder in Program.cs
