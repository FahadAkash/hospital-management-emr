...
function load(uri) {
  ...
  script.src = "/assets-dph/dicom-assets/cornerstone/lib/pako.min.js";
  ...
  var promise = new Promise(function (resolve, reject) {
    loadDICOMPromise.then(function (dicomPart10AsArrayBuffer /* , xhr*/) {
      ...
      var dataSet = void 0;
         ...
         try {
                //Rajesh:24Mar'19
                //Decompression of data using pako
              var output = pako.inflateRaw(byteArray);
             dataSet = dicomParser.parseDicom(output);
            } catch (error) {
        return reject(error);
     }
