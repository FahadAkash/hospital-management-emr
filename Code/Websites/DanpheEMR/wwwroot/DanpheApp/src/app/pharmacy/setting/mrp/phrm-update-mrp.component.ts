...
export class PHRMUpdateMRPComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.oldMRP = this.currentStock.SalePrice;
    }
    Update() {
        if (this.CheckMRP()) {
            if (confirm("Are you Sure want to update SalePrice?")) {
    ...
    CheckMRP() {
        ...
        if (this.currentStock.SalePrice == null) return false;
        if (this.currentStock.SalePrice <= 0) return false;
        return true;
    }
