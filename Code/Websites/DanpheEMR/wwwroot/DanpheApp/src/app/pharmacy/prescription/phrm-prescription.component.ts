...
export class PHRMPrescriptionComponent {
    ...
    ConditionalValidationOfProvider() {
        ...
        if (this.currPrescription.IsInPatient) {
            this.currPrescription.PrescriberName = "";
            let onOff = 'on';
            let formControlName = 'PrescriberId';
            this.currPrescription.UpdateValidator(onOff, formControlName);
            this.currPrescription.PHRMPrescriptionValidator.controls['PrescriberId'].updateValueAndValidity();
        }
        else {
            this.selProvider = null;
            //incase age was entered.
            this.currPrescription.PrescriberId = null;
            let onOff = 'off';
            let formControlName = 'PrescriberId';
            this.currPrescription.UpdateValidator(onOff, formControlName);
            this.currPrescription.PHRMPrescriptionValidator.controls['PrescriberId'].updateValueAndValidity();
    ...
    SavePrescription(): void {      
        ...
        if (this.currPrescriptionItems != null) {
            if (this.currPrescription.IsInPatient) {
                this.currPrescription.PrescriberId = this.selProvider ? this.selProvider.Key : 0;
                this.currPrescription.PrescriberName = this.selProvider ? this.selProvider.Value : '';
                this.IsWrongProvider = (this.currPrescription.PrescriberId > 0) ? false : true;
            } else {
                this.IsWrongProvider = (this.currPrescription.PrescriberName.length > 0 && this.currPrescription.PrescriberName) ? false : true;
            }
