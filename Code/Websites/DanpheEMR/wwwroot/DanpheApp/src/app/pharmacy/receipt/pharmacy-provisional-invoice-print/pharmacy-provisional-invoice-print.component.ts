...
import { PharmacyProvisionalReceipt_DTO } from "./pharmacy-provisional-receipt.dto";

@Component({
    selector: "pharmacy-provisional-invoice-print",
    templateUrl: "./pharmacy-provisional-invoice-print.component.html"
})

export class PharmacyProvisionalInvoicePrintComponent {

    @Input('provisional-invoice') ProvisionalInvoice: PharmacyProvisionalReceipt_DTO = new PharmacyProvisionalReceipt_DTO();
    ...
    */
    public UpdateItemDisplayName(showGenericName: boolean, showItemName: boolean, separator: string = '', showGenericNameAfterItemName: boolean) {
        for (var i = 0; i < this.ProvisionalInvoice.ProvisionalInvoiceItems.length; i++) {
            var item = this.ProvisionalInvoice.ProvisionalInvoiceItems[i];
