...
export class AddDeathDetailsSharedComponent {
    ...
    constructor(public medicalRecordsBLService: MR_BLService,
        ...
        this.GetLatestCertificateNumber();
    }
    ...
    public AddUpdateDeathDetails() {
        if (this.IsPatientDead && this.IsPatientDead == true) {
            this.UpdateDeathDetails();
        }
        else {
            this.AddDeathDetails();
        }
    ...
    public UpdateDeathDetails() {
        var IsDataValid: boolean = this.ValidateDeathDetails();
        if (IsDataValid) {
            this.emitterForDeathDetails.emit({ Status: 'update', data: this.DeathDetails });
            if (this.showSelectPatient == false) {
                this.msgBoxServ.showMessage('success', ["Death Details has been updated successfully"]);
            }
        }
        else {
            this.msgBoxServ.showMessage('error', ["Please Enter Valid Data"])
        }
    }

    public GetLatestCertificateNumber() {
        this.medicalRecordsBLService.GetLatestDeathCertificate().subscribe(
            res => {
                if (res) {
                    this.DeathDetails.CertificateNumber = res;
                }
                else {
                    this.msgBoxServ.showMessage('warning', ["Unable to get latest death certificate number"]);
                }
            }
        );
    }
