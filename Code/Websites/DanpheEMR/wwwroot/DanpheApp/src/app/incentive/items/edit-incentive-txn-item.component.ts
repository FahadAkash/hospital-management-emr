    ...
    if (this.isPercentagesValid && this.checkIfFractionValid()) {
      ...
      this.dlService.Add(data, url).map(res => res).subscribe(res => {
        if (res.Status == "OK") {
          ...
          this.billtxnItem.PerformerId = this.txnItemToEdit.AssignedToEmpId;
          this.billtxnItem.PerformerName = this.txnItemToEdit.AssignedToEmpName;
          this.billtxnItem.PrescriberId = this.txnItemToEdit.ReferredByEmpId;
