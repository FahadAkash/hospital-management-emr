...
export class GoodReceiptEndPoint {
    ...
    public AddGoodReceipt(CurrentReceipt) {
        ...
        let goodReceiptItems = CurrentReceipt.GoodsReceiptItem.map(item => {
            return _.omit(item, ['GoodsReceiptItemValidator', 'OtherChargesModel']);
        });
    ...
    public getINVOtherChargesDetails() {
        return this.http.get<any>("/api/InventoryGoodReceipt/getINVOtherChargesDetails", this.options);
    }
