...
import { GoodsReceiptOtherChargeModel } from '../goods-receipt-other-charges.model';
...
export class GoodsReceiptAddComponent implements OnInit {
  ...
  public IsVerificationActivated: boolean = false;
  ...
  showAddOtherChargesPopUp: boolean = false;
  grItemIndex: number;
  OtherCharges: GoodsReceiptOtherChargeModel;
  otherChargesDetails: any[];
  showOtherChargeLebel: boolean = false;
  IsGRItemNotEditable: boolean = false;
  public GoodReceiptList: Array<GoodsReceipt> = new Array<GoodsReceipt>();
  ...
  ShowGoodsReceiptDetails(res) {
    if (res.Status == "OK") {
      ...
      for (let i = 0; i < goodsReceiptItems.length; i++) {
        ...
        currGRItem.OtherChargesModel.TotalOtherCharge = goodsReceiptItems[i].OtherCharge;
        currGRItem.OtherChargesModel.OtherChargesItem = goodsReceiptItems[i].GRItemCharges;
  ...
  LoadPo(purchaseOrderId: number) {
    this.procurementBLService.GetPurchaseOrderItemsByPOId(purchaseOrderId)
      .subscribe(res => this.LoadPurchaseItemsForGoodsReceipt(res));
  }
  //Load Purchase Order Items as Goods Receipt Items
  LoadPurchaseItemsForGoodsReceipt(res): void {
    this.purchaseOrder = res.Results;
    const POItemStatuses = ['active', 'partial'];
    this.purchaseOrder.PurchaseOrderItems = this.purchaseOrder.PurchaseOrderItems.filter(item => POItemStatuses.includes(item.POItemStatus));
    this.changeDetectorRef.detectChanges();
    ...
    this.goodsReceipt.OrderDate = this.purchaseOrder.PoDate;
  ...
  SelectItemFromSearchBox(Item: ItemMaster, index) {
    if (this.goodsReceipt.GoodsReceiptItem.length > 0) {
      this.ResetItemSelectionRow(index);
      ...
      this.goodsReceipt.GoodsReceiptItem[index].itemPriceHistory = this.inventoryService.allItemPriceList != null ? this.inventoryService.allItemPriceList
        .filter((item) => item.ItemId == Item.ItemId)
        .filter((u, i) => i < 3) : []; //taking top 3
    }
    this.goodsReceipt.updateItemDuplicationStatus();
  }
  //Reset ItemId, ItemRate, VAT and itemPriceHistory if Item Changed
  ResetItemSelectionRow(index) {
    if (!this.editGR && !this.isGrFromPOMode) {
      this.goodsReceipt.GoodsReceiptItem[index].ItemId = 0;
      this.goodsReceipt.GoodsReceiptItem[index].ItemRate = 0;
      this.goodsReceipt.GoodsReceiptItem[index].VAT = 0;
      this.goodsReceipt.GoodsReceiptItem[index].itemPriceHistory = null;
    }
  ...
  Calculations() {
    ...
    this.goodsReceipt.GoodsReceiptItem.forEach(item => {
      ...
      let CcAmount = freeQty * itemRate * CcCharge;
      ...
      this.addOtherChargeToTotalAmount ? totalAmount = totalAmount + vatAmount + item.OtherCharge : totalAmount = totalAmount + vatAmount;


      item.TotalAmount = totalAmount;
      item.CostPrice = totalAmount / totalQty;
    ...
    //.goodsReceipt.OtherCharges = this.goodsReceipt.InsuranceCharge + this.goodsReceipt.CarriageFreightCharge + this.goodsReceipt.PackingCharge + this.goodsReceipt.TransportCourierCharge + this.goodsReceipt.OtherCharge;
    // this.goodsReceipt.TotalAmount += this.goodsReceipt.OtherCharges;
  }

  //add a new row 
  AddRowRequest() {
    let index = this.goodsReceipt.GoodsReceiptItem.length;
    if (index == 0) {
      ...
      if (this.goodsReceipt.GoodsReceiptItem[index - 1].ItemId <= 0) {
        this.messageBoxService.showMessage("Alert!", ["Please select the item first"]);
        return;
      }
      else {
        var goodItem = new GoodsReceiptItems();
  ...
  CheckValidation(): any {
    ...
    if (this.goodsReceipt.GoodsReceiptItem.some(gri => gri.ItemId <= 0)) {
      validationObj.isValid = false;
  ...
  public CheckDuplicateBill() {
    ...
    if (this.goodsReceipt.BillNo && this.vendorBillHistoryList.length > 0) {
      ...
      if (billNo != "" && VendorId > 0 && fiscalyearId != 0) {
        for (let row of this.GoodReceiptList) {
          if (this.editGR == false) {
            if (billNo == row.BillNo && VendorId == row.VendorId && fiscalyearId == row.FiscalYearId) {
  ...
  public get IsGoodsReceiptDateValid(): boolean {
    return this.inventoryService.allFiscalYearList.some(
      fy =>
        (fy.IsClosed == null || fy.IsClosed == false)
        &&
        moment(this.goodsReceipt.GoodsReceiptDate).isBetween(fy.StartDate, fy.EndDate, null, '[]')
    );
  }
  ...
  onPressedEnterKeyInRateField(index: number) {
    ...
    if (this.isGrFromPOMode) {
      this.GoToNextInput(`qtyip${index + 1}`, 300);
    }
    else {
  ...
  getINVOtherChargesDetails() {
    this.goodReceiptService.getINVOtherChargesDetails()
      .subscribe(res => {
        if (res.Status == "OK") {
  ...
  addOtherCharges(index) {
    this.showAddOtherChargesPopUp = true;
    this.grItemIndex = index;
    this.OtherCharges = this.goodsReceipt.GoodsReceiptItem[index].OtherChargesModel;

  }
  OnOtherChargesClosedEventHandler() {
    this.showAddOtherChargesPopUp = false;
  }
  OnOtherChargesAddedEventHandler($event) {
    this.showAddOtherChargesPopUp = false;
    this.goodsReceipt.GoodsReceiptItem[$event.grItemIndex].OtherCharge = $event.otherCharges.TotalOtherCharge;
    this.goodsReceipt.GoodsReceiptItem[$event.grItemIndex].OtherChargesModel = $event.otherCharges;
    this.goodsReceipt.GoodsReceiptItem[$event.grItemIndex].OtherChargesModelList = $event.otherCharges.OtherChargesItem.filter(a => a.TotalAmount > 0);
    this.CalculateOtherCharges();
  }

  public OtherChargeDetails: any[];
  CalculateOtherCharges() {
    let totalChargeInGR = this.goodsReceipt.OtherChargesList = [];

    for (var grItem of this.goodsReceipt.GoodsReceiptItem) {
      for (var charge of grItem.OtherChargesModel.OtherChargesItem) {
        charge = JSON.parse(JSON.stringify(charge));
        if (totalChargeInGR.some(a => a.ChargeId == charge.ChargeId)) {
          const matchingCharge = totalChargeInGR.find(a => a.ChargeId == charge.ChargeId);
          matchingCharge.VATAmount += charge.VATAmount;
          matchingCharge.Amount += charge.Amount;
          matchingCharge.TotalAmount += charge.TotalAmount;
          continue;
        }
        totalChargeInGR.push(charge);
      }
    }
    this.goodsReceipt.OtherChargesList = this.goodsReceipt.OtherChargesList.filter(charge => charge.TotalAmount > 0);
    this.goodsReceipt.OtherCharges = this.goodsReceipt.OtherChargesList.reduce((a, b) => a + b.TotalAmount, 0);
  }

  //Use to get all the goods receipt list to check, so that no duplicate invoice no is created with same supplier and  within same fiscal year. 
  getGoodsReceiptMasterList() {
    this.procurementBLService.getGoodsReceiptMasterList().subscribe(res => {
      if (res.Status == "OK") {
        this.GoodReceiptList = res.Results;
        this.GoodReceiptList = this.GoodReceiptList.filter(gr => gr.IsCancel == false);
      }
      else {
        this.messageBoxService.showMessage('error', ['Failed to load the GoodsReceipt list']);
      }
    }, err => {
      this.messageBoxService.showMessage('error', ['Failed to load the GoodsReceipt list']);
    })
  ...
  CheckIfOtherChargeAddToTotalAmount() {

    let GROtherChargeParameter = this.coreService.Parameters.find(p => p.ParameterName == "GROtherChargesFormCustomization" && p.ParameterGroupName == "Procurement");
    if (GROtherChargeParameter != null) {
      let GRParameter = JSON.parse(GROtherChargeParameter.ParameterValue);
      if (GRParameter.addOtherChargeInGRTotalAmount == true) {
        this.addOtherChargeToTotalAmount = true;
      }
      else {
        this.addOtherChargeToTotalAmount = false;
      }
    }
