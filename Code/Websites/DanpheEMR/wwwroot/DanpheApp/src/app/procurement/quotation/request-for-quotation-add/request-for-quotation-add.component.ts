...
export class RequestForQuotationAddComponent implements OnInit {
  ...
  AddRequestForQuotaion() {
    ...
    if (CheckIsValid == true && this.ReqForQuotation.ReqForQuotationItems != null && this.ReqForQuotation.ReqForQuotationVendors != null) {
      ...
      if (this.ReqForQuotation.Subject.trim() == "" || this.ReqForQuotation.Description.trim() == "") {
        this.messageBoxService.showMessage('Alert', ['Please fill the mendatory field. ']);
        return;
      }
      if (this.ReqForQuotation.ReqForQuotationVendors.some(ven => ven.VendorId == 0 || ven.VendorId == null)) {
        this.messageBoxService.showMessage('Alert', ['Please select vendor. ']);
        return;
      }
      if (this.ReqForQuotation.ReqForQuotationItems.some(itm => itm.Quantity == 0 || itm.Quantity == null)) {
        this.messageBoxService.showMessage('Alert', ['Quantity should be greater than Zero. ']);
        return;
      }
      this.loading = true;
      this.quotationBLService.PostToReqForQuotation(this.ReqForQuotation).finally(() => { this.loading = false }).
        subscribe(res => {
          if (res.Status == 'OK') {
