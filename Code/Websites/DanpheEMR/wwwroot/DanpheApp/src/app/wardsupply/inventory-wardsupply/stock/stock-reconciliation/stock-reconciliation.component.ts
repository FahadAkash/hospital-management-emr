...
@Component({
  selector: 'ward-stock-reconciliation',
  templateUrl: './stock-reconciliation.component.html',
  styleUrls: ['./stock-reconciliation.component.css']
})
export class StockReconciliationComponent {

  @Input('stock-list') stockList: Array<WardStockModel> = new Array<WardStockModel>();
  ...
  constructor(public msgBoxServ: MessageboxService, public wardSupplyBLservice: WardSupplyBLService) { }
  ...
  ExportStocksForReconciliationToExcel() {
    ...
    this.wardSupplyBLservice.ExportStocksForReconciliationToExcel(this.StoreId)
      .finally(() => { this.showSpinner = false; })
      .subscribe(res => {
        let blob = res;
        let a = document.createElement("a");
        a.href = URL.createObjectURL(blob);
        a.download = "SubstoreStockReconciliation_" + moment().format("DD-MMM-YYYY_HHmmA") + '.xlsx';
  ...
  UpdateReconciledStockFromExcelFile() {
    this.loading = true;
    this.wardSupplyBLservice.UpdateReconciledStockFromExcelFile(this.reconciledStocksWithQuantityChanges).finally(() => {
      this.loading = false;
      this.confirmPopUp = false;
    }).subscribe(res => {
      if (res.Status == "OK") {
