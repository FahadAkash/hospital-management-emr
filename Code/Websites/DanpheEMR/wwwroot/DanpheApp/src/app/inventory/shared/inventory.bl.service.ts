...
export class InventoryBLService {
  ...
  GetDispatchItemByDispatchId(DispatchId: number, RequisitionId: number, CreatedOn?) {
    return this.inventoryDLService.GetDispatchItemByDispatchId(DispatchId, RequisitionId, CreatedOn)
      .map((responseData) => {
        return responseData;
      });
  }
  ...
  PostToDispatchItems(dispatch: Dispatch) {
    const dispatchItemsTemp = dispatch.DispatchItems.map(item => {
      const { DispatchItemValidator, ...rest } = item;
      return rest;
    });
  ...
  public GetAvailableQuantityByItemIdAndStoreId(ItemId: number, StoreId: number) {
    return this.inventoryDLService.GetAvailableQuantityByItemIdAndStoreId(ItemId, StoreId)
      .map((responseData) => {
        return responseData;
      });
  }
  public GetReturnItemsFromSubstore(fromdate: string, todate: string, targetstoreId: number, sourceSubstoreId: number) {
    return this.inventoryDLService.GetReturnItemsFromSubstore(fromdate, todate, targetstoreId, sourceSubstoreId)
      .map((responseData) => {
        return responseData;
      });
  }
  public GetAllSubStores() {
    return this.inventoryDLService.GetAllSubstores()
      .map((responseData) => {
        return responseData;
      });
  }
  public ReceiveRetunedItems(returnId: number, retunedRemarks: string) {
    return this.inventoryDLService.ReceiveRetunedItems(returnId, retunedRemarks)
      .map((responseData) => {
        return responseData;
      });
  }
  public GetPurchaseOrderDraftById(purchaseOrderDraftId: number) {
    return this.inventoryDLService.GetPurchaseOrderDraftById(purchaseOrderDraftId)
      .map((responseData) => {
        return responseData;
      });
  }
