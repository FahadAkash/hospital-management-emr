...
export class DailyItemDispatchComponent {
  ...
  TotalDispatchedValue: number = 0;
  TotalDispatchedQuantity: number = 0;
  ...
  Error(err) {
    this.DailyItemDispatchReportData = [];
    this.ListFiltered = [];
    this.CurrentItemDispatch.StoreId = null;
    this.TotalDispatchedQuantity = 0;
    this.TotalDispatchedValue = 0;
  ...
  Success(res) {
    if (res.Status == "OK" && res.Results.length > 0) {
      ...
      this.TotalDispatchedValue = 0;
      this.CurrentItemDispatch.StoreId = null;
      this.selectedstore = this.CurrentItemDispatch.SelectedStore;
  ...
  CalculateTotal(): void {
    this.TotalDispatchedQuantity = this.TotalDispatchedValue = 0;
    this.TotalDispatchedQuantity = this.DailyItemDispatchReportData.reduce((a, b) => a + b.DispatchedQty, 0);
    this.TotalDispatchedValue = this.DailyItemDispatchReportData.reduce((a, b) => a + b.TotalDispatchedValue, 0);
  }
}
