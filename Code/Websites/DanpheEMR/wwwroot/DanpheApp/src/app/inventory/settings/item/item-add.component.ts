...
export class ItemAddComponent {
  ...
  IsFixedAssets: boolean = false;
  isSelectedCategoryCapitalGoods: ItemCategoryModel;
  ...
  constructor(
    ...
    this.checkIsFixedAssetsApplicableInCapitalGoodsItem();
  }
  ...
  private getAllRoles() {
    this.invSettingBL.getActiveRbacRoles().subscribe(res => {
      if (res.Status == "OK") {
    ...
    this.invSettingBL.GetItemCategory()
      .subscribe(res => {
        if (res.Status == 'OK') {
          ...
          this.isSelectedCategoryCapitalGoods = this.ItemCategoryList.find(x => x.ItemCategoryName == "Capital Goods");
        } else {
          this.msgBoxServ.showMessage("error", [res.ErrorMessage]);
        }
      });
  ...
  CallBackAddItem(res) {
    if (res.Status == "OK") {
      ...
      let inventory = this.activeInventories.find(i => i.StoreId == item.StoreId);
      if (inventory) {
        item.StoreName = inventory.Name;
      }
  ...
  public UpdateRegisterPageNumberValidator() {
    try {
      ...
      if (this.CurrentItem && this.CurrentItem.ItemCategoryId > 0 && this.registerNoDisplayParam && this.showRegisterPageNumber == true) {
  ...
  checkIsFixedAssetsApplicableInCapitalGoodsItem() {
    let IsFixedAssetsApplicableInCapitalGoodsItem = this.coreService.Parameters.find(p => p.ParameterName == "IsFixedAssetsApplicableInCapitalGoodsItem" && p.ParameterGroupName == "Procurement");
    if (IsFixedAssetsApplicableInCapitalGoodsItem != null) {
      let IsFixedAssetsParameter = JSON.parse(IsFixedAssetsApplicableInCapitalGoodsItem.ParameterValue);
      this.IsFixedAssets = IsFixedAssetsParameter.IsFixedAssets;
