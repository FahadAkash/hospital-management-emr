...
export class InternalMainPurchaseRequestAddComponent {
  ...
  OnPRCategoryChanged(indx) {
    ...
    this.GoToNextInput("itemName" + indx, 100, indx);
  }
      ...
      .subscribe(res => {
        if (res.Status == "OK") {
          ...
          for (var i = 0; i < RequestsItemArray.length; i++) {
            ...
            this.currentPurchaseRequest.PurchaseRequestItems[i].ItemCategory = RequestsItemArray[i].ItemCategory;
  ...
  SelectItemFromSearchBox(Item: ItemModel, index) {
    ...
    if (typeof Item === "object" && !Array.isArray(Item) && Item !== null) {
      ...
      if (this.checkIsItemPresent == true) {
        ...
        this.currentPurchaseRequestItems.ItemCategory = ENUM_GRItemCategory.Consumables;
        this.currentPurchaseRequestItems.filteredItemList = this.GetItemListByItemCategory(this.currentPurchaseRequestItems.ItemCategory);
        ...
        this.setFocusById('itemName' + (this.currentPurchaseRequest.PurchaseRequestItems.length - 1));
      ...
      this.currentPurchaseRequest.PurchaseRequestItems[index].ItemId = null;
      this.currentPurchaseRequest.PurchaseRequestItems[index].SelectedItem = null;
    }
  }
      ...
      this.inventoryBLService.PostToPORequisition(this.currentPurchaseRequest)
        ...
        .subscribe(res => {
          if (res.Status == 'OK') {
            ...
            this.inventoryService.PurchaseRequestId = res.Results;//sud:3Mar'20-Property Rename in InventoryService
  ...
  OnNewItemAdded($event) {
    ...
    if (this.currentPurchaseRequest.PRCategory == item.ItemType) {
      ...
      this.currentPurchaseRequest.PurchaseRequestItems[this.index].ItemCategory = item.ItemType;
    }
    else {
  ...
  OnNewVendorAdded($event) {
    ...
    this.inventoryService.LoadAllVendorList(this.VendorList);
    this.selectedVendor = newVendor;
    this.SelectVendorFromSearchBox();
  }
  GoToNextInput(id: string, focusDelayInMs: number = 0, index) {
    if (this.currentPurchaseRequest.PurchaseRequestItems[index].ItemId != null && this.currentPurchaseRequest.PurchaseRequestItems[index].ItemId > 0) {
