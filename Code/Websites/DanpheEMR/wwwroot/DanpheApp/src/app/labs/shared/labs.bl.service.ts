...
export class LabsBLService {
  ...
  public GetLabWorkListData(fromDate: string, toDate: string, catIdList: Array<number> = []) {
    const catListStr = catIdList.join(',');//we need to send comma separated value to server api.
    return this.labDLService.GetLabWorkListData(fromDate, toDate, catListStr).map((res) => {
      return res;
    });
  }
  ...
  GetAllIMUData(fromDate: Date, toDate: Date) {
    return this.labDLService.GetAllImuData(fromDate, toDate).map((res) => {
      return res;
    })
  }

  PostDataToIMU(data: any) {
    let dataJson = JSON.stringify(data);
    return this.labDLService.PostDataToIMU(dataJson).map((res) => {
      return res;
    })
  }
  GetMembershipWiselabTestCount(fromDate: string, toDate: string) {
    return this.labDLService.GetMembershipWiselabTestCount(fromDate, toDate).map((res) => {
      return res;
    })
  }
  GetRankWiselabTestCount(fromDate: string, toDate: string) {
    return this.labDLService.GetRankWiselabTestCount(fromDate, toDate).map((res) => {
      return res;
    })
  }
  GetTrendinglabTestCount(fromDate: string, toDate: string) {
    return this.labDLService.GetTrendinglabTestCount(fromDate, toDate).map((res) => {
      return res;
    })
  }
  GetLabTestDoneTodayDetails() {
    return this.labDLService.GetLabTestDoneTodayDetails().map((res) => {
      return res;
    })
  }
  GetDengueTestDetails() {
    return this.labDLService.GetDengueTestDetails().map((res) => {
      return res;
    })
  }

  GetTestReqDetails() {
    return this.labDLService.GetLabReqDetails().map((res) => {
      return res;
    })
  }
  GetTestNormalAbnormalDetails(testId: number) {
