...
export class RPT_BIL_ReturnBillModel {
    ...
    public ReturnBillValidator: FormGroup = null;

    constructor() {

        var _formBuilder = new FormBuilder();
        this.ReturnBillValidator = _formBuilder.group({
    ...
    public IsDirty(fieldName): boolean {
        if (fieldName == undefined)
            return this.ReturnBillValidator.dirty;
        else
            return this.ReturnBillValidator.controls[fieldName].dirty;
    }

    public IsValid():boolean{if(this.ReturnBillValidator.valid){return true;}else{return false;}}
     public IsValidCheck(fieldName, validator): boolean {
        if (fieldName == undefined)
            return this.ReturnBillValidator.valid;
        else
            return !(this.ReturnBillValidator.hasError(validator, fieldName));
    }
}
