...
export class RPT_BIL_DailyMISReportModel {
    public fromDate: string = "";
    public toDate: string = "";
    public DailyMISReportValidator: FormGroup = null;

    constructor() {
        var _formBuilder = new FormBuilder();
        this.DailyMISReportValidator = _formBuilder.group({
    ...
    public IsDirty(fieldName): boolean {
        if (fieldName == undefined)
            return this.DailyMISReportValidator.dirty;
        else
            return this.DailyMISReportValidator.controls[fieldName].dirty;
    }

    public IsValid():boolean{if(this.DailyMISReportValidator.valid){return true;}else{return false;}} public IsValidCheck(fieldName, validator): boolean {
        if (fieldName == undefined)
            return this.DailyMISReportValidator.valid;
        else
            return !(this.DailyMISReportValidator.hasError(validator, fieldName));
    }
} 
